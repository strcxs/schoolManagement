...
@section('content')
<div class="container">
    <h1 class="text-center mb-4">Agenda Scheduler</h1>
    <form id="addScheduleForm" class="mt-4 mb-2">
        <div class="row g-3">
            <div class="col-md-4">
                <input type="text" id="nama_schedule" name="nama_schedule" class="form-control" placeholder="nama Schedule" required>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                <button type="submit" class="btn btn-success w-100">Tambah Schedule</button>
            </div>
        </div>
    </form>
    <hr>
        ...
        <table id="guruTable" class="table table-striped table-bordered">
            ...
            <tbody id="guruList">
                @foreach ($schedule as $x)
                    <tr>
                        <td>{{$x->nama_schedule}}</td>
                        <td>{{$x->mapel->nama}}</td>
                        <td>{{$x->guru->nama ?? '-'}}</td>
                        <td>
                            <!-- Edit Button -->
                            <a class="btn btn-warning btn-sm" href="{{ route('schedule.editSchedule', ['data' => $x->id]) }}">
                                Edit
                            </a>
...
<script>
    $(document).ready(function() {
        ...
        $('#addScheduleForm').on('submit', function(e) {
            ...
            var form = $(this);
            var actionUrl = "{{ route('schedule.add') }}"; // URL untuk menambah guru
